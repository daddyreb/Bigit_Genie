...
bool SmartWifiParser::parser_get_enable(QString& xml, bool& enable, QString& status_name, QString& waittime, QString& error)
{
	QDomDocument doc;
	doc.setContent(xml);
		...
		if (! ele1.isNull())
		{
			...
			if (! ele2.isNull())
			{
				...
				if (! ele3.isNull())
				{
					...
					ele4 = ele3.firstChildElement("wmtime");
					if (! ele4.isNull())
					{
						waittime = ele4.text();
					}
...
bool SmartWifiParser::parser_del_white_url(QString& xml, QString& error)
{
	QDomDocument doc;
	doc.setContent(xml);
	QDomElement root = doc.documentElement();
...
bool SmartWifiParser::parser_get_white_url_page(QString& xml, QStringList& white_url, QString& error)
{
	QDomDocument doc;
	doc.setContent(xml);
	QDomElement root = doc.documentElement();
	...
	QDomElement ele2 = ele1.firstChildElement("svpn:get-whitelist-pgResponse");
	if (ele2.isNull())
		return false;

	QDomElement ele3 = ele2.firstChildElement("retWhitelist");
	if (ele3.isNull())
		return false;

	QDomElement ele4 = ele3.firstChildElement("string");
	if (ele4.isNull())
		return false;

	white_url = ele4.text().split(";");
	white_url.removeAll(QString(""));

	QDomElement ele5 = ele3.firstChildElement("result");
	if (ele5.isNull())
		return false;
