...
bool* Qrcode::calQrcode(char* abyte0,int iLen0,int& iLenRet)
{
	...
	/*ai1 = (new int[] {
        ...
        try
        {
            ...
            QString s;
//			s.Format(":/qrcode_data/qrv%d_%d.dat",,);
            s=QString(":/qrcode_data/qrv%1_%2.dat").arg(qrcodeVersion).arg((int)byte1);
        ...
        try
        {
            ...
            QString s1;
            s1=QString(":/qrcode_data/qrvfr%1.dat").arg(qrcodeVersion);
            QFile f1(s1);
            Q_ASSERT(f1.exists());
...
//		}
        ...
        memset(abyte15,'\0',l3*l3*sizeof(abyte15[0]));
...
//		}
		...
		for(int l4 = 0; l4 < k2; l4++)
		{
			...
			for(int j5 = 7; j5 >= 0; j5--)
			{
                ...
                abyte15[(abyte2[l5] & 255)*l3+(abyte3[l5] & 255)] = (char)(255 * (byte3 & 1) ^ abyte4[l5]);
		...
		{
			int k5 = (i5 + k2 * 8) - 1;
            abyte15[(abyte2[k5] & 255)*l3+(abyte3[k5] & 255)] = (char)(255 ^ abyte4[k5]);
		}
		...
		{			
            ...
            abyte15[(abyte10[i6] & 255)*l3+(abyte11[i6] & 255)] = (char)(byte7 * 255);
            abyte15[(abyte5[i6] & 255)*l3+(abyte6[i6] & 255)] = (char)(byte7 * 255);
		}

		//boolean [][]aflag1 = new boolean[l3][l3];
        bool* aflag1 = new bool[l3*l3];	//\\\\\\\\\\\
//		for( n1 = 0; n1 <l3; n1++)
//		{
...
//		}
		...
		for(int k6 = 0; k6 < l3; k6++)
		{
			...
			{
                if((abyte15[l6*l3+k6] & byte5) != 0 || abyte12[j6] == 49)
                    aflag1[l6*l3+k6] = true;
				else
                    aflag1[l6*l3+k6] = false;
				j6++;
			}
...
//void Qrcode::destroyMemory(bool **s, int len)
//{
//    for(int i=0;i<len;i++)
//    {
//        if(s[i])
...
char* Qrcode::calculateRSECC(char* abyte0, char byte0, char* abyte1, int i, int j,
	...
	//iLen1 abyte1.length  ; iLen0 abyte0.length
        ...
        {            
            QString s;
            s=QString(":/qrcode_data/rsc%1.dat").arg((int)byte0);
//			s.Format( ,byte0);
            QFile f1(s);
            Q_ASSERT(f1.exists());
            f1.open(QIODevice::ReadOnly);
//			CFile f1(s, CFile::modeRead);
    ...
    //private 
char* Qrcode::calculateByteArrayBits(char* abyte0, char* abyte1, QString s,
...
if(l1 > 0 && i1 > 0)
{
    k = abyte0[l1*iLen0+i1] & abyte0[(l1 - 1)*iLen0+i1] & abyte0[l1*iLen0+(i1 - 1)] & abyte0[(l1 - 1)*iLen0+(i1 - 1)] & 255;
    l = abyte0[l1*iLen0+i1] & 255 | abyte0[(l1 - 1)*iLen0+i1] & 255 | abyte0[l1*iLen0+(i1 - 1)] & 255 | abyte0[(l1 - 1)*iLen0+(i1 - 1)] & 255;
}
      ...
      for(int j2 = 0; j2 < 8; j2++)
			{
                ...
                int n=abyte0[l1*iLen0+i1] & 255;
				int x=j2;				

				ai5[j2] = (ai5[j2] & 63) << 1 |(n>>x)&((int)pow((double)2,32-x)-1) & 1;
				ai6[j2] = (ai6[j2] & 63) << 1 |(n>>x)&((int)pow((double)2,32-x)-1) & 1;
				//ai5[j2] = (ai5[j2] & 63) << 1 |((abyte0[l1][i1] & 255))>> j2 & 1;
...
//http://wenku.baidu.com/view/8482cfd0b9f3f90f76c61b82.html
void Qrcode::autoSetOptions(int dataLen)
{
    const int cParas[][4]=
    {
        //L  M  Q  H
    ...
    const char cErrorCorrect[4]={'L','M','Q','H'};
    for(int i=0;i<sizeof(cParas)/sizeof(cParas[0]);i++)
    {
        for(int j=3;j>=0;j--)
        {
            if(cParas[i][j]>dataLen)
            {
                setQrcodeErrorCorrect(cErrorCorrect[j]);
                setQrcodeEncodeMode('B');
                setQrcodeVersion(i);
                return;
            }
        }
