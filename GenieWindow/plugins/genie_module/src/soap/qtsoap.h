...
enum NetgearResponseEnum
{
	...
	VALUE_SPECIFIED_IS_INVALID	=	702
};
...
public:
    ...
    void setFlag(bool f){QtSoapSimpleType::flag = f;}
	...
	const QtSoapType &returnResponseCode() const;
...
public:
    ...
    void setHost(const QString &host, QHttp::ConnectionMode mode, int port = 0);
    void setAction(const QString &action);
    void submitRequest(QtSoapMessage &request, const QString &path);
    void submitRequest(const QString &request, const QString &path);
    const QtSoapMessage &getResponse() const;

    const QHttp *httpDriver() const;
