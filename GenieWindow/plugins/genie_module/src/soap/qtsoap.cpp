...
bool QtSoapSimpleType::flag=false;
/*!
...
//    qDebug() << "tmpe3" << tmpe3.text();
    ...
    // correct.
//    qDebug() << "tmpe.namespaceURI()" << tmpe.namespaceURI();
...
//		     XML_SCHEMA);
    env.setAttribute("xmlns:SOAP-ENV",SOAPv11_ENVELOPE);
...
*/
const QtSoapType &QtSoapMessage::returnResponseCode() const
{
...
QtSoapHttpTransport::QtSoapHttpTransport(QObject *parent)
    : QObject(parent), http(this), requestid(0)
{
    connect(&http, SIGNAL(requestFinished(int, bool)), SLOT(readResponse(int, bool)));
...
/*!
    Sets the \a host and \a port this transport should connect to.
    Also implicitly sets the HTTP header HOST to \a host.
*/
...
/*!
  ...
  Sets the \a host and \a port this transport should connect to, and
  the transport connection mode to \a mode. If \a port is 0 (the
  default), the default port number according to the transport mode is
  used (80 for HTTP, 443 for HTTPS).

  Also implicitly sets the HTTP header HOST to \a host.
*/
void QtSoapHttpTransport::setHost(const QString &host, QHttp::ConnectionMode mode, int port)
{
...
{
    header.setValue("SOAPAction", action);
}
...
{
    header.setRequest("POST", path, 1, 0);

    QString b = request.toXmlString();
    header.setValue("User-Agent","SOAP Toolkit 3.0");
    header.setValue("Host","www.routerlogin.com");
    header.setValue("connection","keep-Alive");
    header.setValue("Cache-Control","no-cache");
    header.setValue("Pragma","no-cache");

//    b = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?><SOAP-ENV:Envelope xmlns:SOAPSDK1=\"http://www.w3.org/2001/XMLSchema\" xmlns:SOAPSDK2=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:SOAPSDK3=\"http://schemas.xmlsoap.org/soap/encoding/\" xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\"><SOAP-ENV:Header><sessionID>12345678901234567890</sessionID></SOAP-ENV:Header><SOAP-ENV:Body><M1:GetInfo xmlns:M1=\"urn:NETGEAR-ROUTER:service:DeviceInfo:1\"/></SOAP-ENV:Body></SOAP-ENV:Envelope>";

    QByteArray a = b.toUtf8().constData();

    response.clear();
    requestid = http.request(header, a);
}

void QtSoapHttpTransport::submitRequest(const QString &request, const QString &path)
{
    header.setRequest("POST", path, 1, 0);
...
/*!
    Returns a pointer to the response SOAP message. This message could
    be a Fault message, so it is wise to check using
...
/*!
    Returns a pointer to the QHttp object used by this transport. This
    is useful if the application needs access to the signals emitted
    by QHttp.
*/
const QHttp *QtSoapHttpTransport::httpDriver() const
{
    return &http;
}
...
*/
void QtSoapHttpTransport::readResponseHeader(const QHttpResponseHeader &resp)
{
...
*/
void QtSoapHttpTransport::readResponse(int id, bool error)
{
    if (id != requestid)
	return;
    ...
    if (error) {
        response.setFaultCode(QtSoapMessage::Client);
        response.setFaultString("Transport failed: " + http.errorString());
    }
    else {
        response.setContent(http.readAll());
    }
...
/*! \class QtSoapNamespaces qtsoap.h
    ...
    given namespace, or it will optionally create a new prefix.

    To access the QtSoapNamespaces registry, call
    QtSoapNamespaces::registry().

    \code
    QtSoapNamespaces &registry = QtSoapNamespaces::registry();
...
/*!
    Returns the prefix for the namespace \a uri. If the namespace
    has not been registered, it is implicitly registered with a
    generated prefix.
*/
QString QtSoapNamespaces::prefixFor(const QString &uri)
{
    ...
    // ### Only generates a0-z9 prefices.
    QString prefix;
    /*
    for (char i = 'a'; i < 'z'; ++i)
        for (char j = '0'; j < '9'; ++j) {
	    prefix.sprintf("%c%c", i, j);
