...
QGenie_MFNetTraffic::QGenie_MFNetTraffic()
{
	lasttraffic = 0.0;
	CurrentInterface = -1;
        //CurrentTrafficType = AllTraffic;
        CurrentTrafficType =OutGoingTraffic;
	GetInterfaces();
}
...
double QGenie_MFNetTraffic::GetTraffic(int interfaceNumber)
{
	try
	{
#define DEFAULT_BUFFER_SIZE 40960L
               ...
               // pos = Interfaces.at(interfaceNumber);
                //if(pos==NULL)
                        //return 0.0;
		//得到当前的接口名字
                //InterfaceName = Interfaces.GetAt(pos);
                InterfaceName = Interfaces.value(interfaceNumber);
		//为性能数据缓冲
		...
		*/
		...
		while((ret = RegQueryValueEx(
                        HKEY_PERFORMANCE_DATA, s_key, NULL, &type, data, &size)) != ERROR_SUCCESS)
		{
			...
			if(objectPtr->ObjectNameTitleIndex == 510) 
			{
				...
				for(int b=0 ; b<objectPtr->NumInstances ; b++) 
				{
					...
					// 现在我们得到了接口的名字
                                        ...
                                        //iName=QString(pName);
                                        iName=QString::fromWCharArray(namePtr);
						...
						fullTraffic = *((DWORD *) ((BYTE *)counterBlockPtr + processIdOffset));
                                                ...
                                                TotalTraffics.insert(b,fullTraffic);
                                        //}
...
//枚举安装的接口
BOOL QGenie_MFNetTraffic::GetInterfaces()
{
	try
	{
#define DEFAULT_BUFFER_SIZE 40960L
			...
			if(objectPtr->ObjectNameTitleIndex == 510) 
			{
				...
				for(int b=0 ; b<objectPtr->NumInstances ; b++) 
				{
					...
					DWORD tottraff = 0;
                                        ...
                                        //Interfaces.insert(QString(pName));
                                        //Interfaces.insert(b,QString(pName));
                                        Interfaces.insert(b,QString::fromWCharArray(namePtr));


                                        Bandwidths.insert(b,bandwith);
                                        TotalTraffics.insert(b,tottraff);  // initial 0, just for creating the list
...
//返回安装的接口数
int QGenie_MFNetTraffic::GetNetworkInterfacesCount()
{
...
//返回接口名称
BOOL QGenie_MFNetTraffic::GetNetworkInterfaceName(QString InterfaceName, int index)
{
        ...
        //InterfaceName->Format("%s",Interfaces.GetAt(pos));
         InterfaceName=Interfaces.value(index);
	return TRUE;
}


//返回接口带宽
DWORD QGenie_MFNetTraffic::GetInterfaceBandwidth(int index)
{
        ...
        return Bandwidths.value(index) ;

}

//具体接口接收和发送的数据量
DWORD QGenie_MFNetTraffic::GetInterfaceTotalTraffic(int index)
{
	DWORD		totaltraffic = 0;
        ...
        //{
                ...
                totaltraffic = TotalTraffics.value(index);
		if(totaltraffic == 0.0)
		{
			GetTraffic(index);
                        //pos= TotalTraffics.FindIndex(index);
...
void QGenie_MFNetTraffic::SetTrafficType(int trafficType)
{
