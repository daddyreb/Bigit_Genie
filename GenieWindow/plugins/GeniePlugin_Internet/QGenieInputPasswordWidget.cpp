...
QGenieInputPasswordWidget::QGenieInputPasswordWidget(QWidget *parent,bool bIndependent)
    :QGenieDialogInnerWidget(parent),m_bIndependent(bIndependent)
{
    init_widget();
}

QGenieInputPasswordWidget::QGenieInputPasswordWidget(const QString &auth,const QString &cipher,QWidget *parent,bool bIndependent)
...
void QGenieInputPasswordWidget::retranslateUi()
{
    if (m_bIndependent)
    {
        ok_btn->setText(/*tr("Ok")*/
                GeniePlugin_Internet::translateText(L_INTERNET_OK_BTN_TEXT));
        cancel_btn->setText(/*tr("Cancel")*/
                GeniePlugin_Internet::translateText(L_INTERNET_IPW_CANCEL_BTN_TEXT));
        opt_result_lb->setText(/*tr("The password is invalid please try again.")*/
                GeniePlugin_Internet::translateText(L_INTERNET_OPTRESULT_LB_TEXT));
    }

    prompt_lb->setText(/*tr("Connect to this network require a password.")*/
            GeniePlugin_Internet::translateText(L_INTERNET_OPTPROMPT_LB_TEXT));


    show_plaintext_cb->setText(/*tr("Show clearly")*/
            GeniePlugin_Internet::translateText(L_INTERNET_SHOWCLEARLY_LB_TEXT));
    input_lb->setText(/*tr("Password:")*/
            GeniePlugin_Internet::translateText(L_INTERNET_INPUTPWD_LB_TEXT));
...
//        opt_result_lb->setVisible(true);
        ...
        QGenieDialogContainerFrame::passwordInvalidBox(this,GeniePlugin_Internet::translateText(L_INTERNET_IPW_PWDINVALID_PROMPT_TITLE),
                                                       GeniePlugin_Internet::translateText(L_INTERNET_IPW_PWDINVALID_PROMPT_TEXT).arg(str_arg)
...
{
    return GeniePlugin_Internet::translateText(L_INTERNET_IPW_DLG_TITLE);
}
