...
#ifdef WIN32
int UdtFile::ListDir(const wchar_t * path, __int64 & nTotalSize, std::vector<_FileInfo>& vecEntries)
{
	// check the arguments
	...
	wstring szPath = path;
	if (szPath.compare(szPath.size()-1, 1, L"\\") == 0 || szPath.compare(szPath.size()-1, 1, L"/") == 0)
		szPath += L"*";
	else
		szPath += L"\\*";
	...
	HANDLE find_handle = FindFirstFileW(szPath.c_str(), &find_data);
	if (find_handle == INVALID_HANDLE_VALUE)
	{
		struct _stat64 stat_buffer;
		TCHAR strAPath[MAX_PATH];
		char strUPath[MAX_PATH];
		wsprintf(strAPath, L"%s", path);
		sprintf_s(strUPath, "%s", WIN32_WC2C(path));
		int nRet = _wstat64(path, &stat_buffer);
		if (nRet == 0)
		{
	...
	do 
	{
		if (WIN32_WC2C(find_data.cFileName) != NULL)
		{
			...
			struct _stat64 stat_buffer;
			TCHAR strAPath[MAX_PATH] = L"";
			char strUPath[MAX_PATH] = "";
			wsprintf(strAPath, L"%s%s%s", path, L"/", find_data.cFileName);
			sprintf_s(strUPath, "%s/%s", path, WIN32_WC2CU(find_data.cFileName));
			int nRet = _wstat64(strAPath, &stat_buffer);
			if (nRet == 0)
			{
				if (S_ISDIR(stat_buffer.st_mode))
				{
...
int UdtFile::CreateDir(const wchar_t* path)
{
	int nLen = 0, nPos = 0;
	wstring szFolder = L"", szSlash = L"";
	wstring szTmpPath = path;
	...
	while (true)
	{
		...
		if (nPos >= 0)
		{
			...
			if (nPos >= 0)
			{
				...
				szFolder = szTmpPath.substr(0, nLen);
#ifdef WIN32
				if (_waccess(szFolder.c_str(), 0))
				{
					BOOL result = ::CreateDirectoryW(szFolder.c_str(), NULL);
					if (result == 0)
						return 1;
