...
int CUdtCore::StartListen(const int nCtrlPort, const int nFilePort)
{
	...
	char strPort[32];
	memset(strPort, 0, 32);
	sprintf_s(strPort, "%d", m_nCtrlPort);
...
int CUdtCore::SendFiles(const char* pstrAddr, std::vector<std::wstring> vecFiles, const char* owndevice, const char* owntype, const char* recdevice, const char* rectype, const char* pstrSendtype,int &iResult)
{
	if (vecFiles.empty())
		return 0;
	...
	for (std::vector<std::wstring>::const_iterator it = vecFiles.begin(); it != vecFiles.end(); it++)
	{
		szTmp = *it;
		if ('\\' == szTmp[szTmp.size()-1] || '/' == szTmp[szTmp.size()-1])
			szTmp = szTmp.substr(0, szTmp.size()-1);
		UdtFile::ListDir(szTmp.c_str(), nFileTotalSize, info);
	}
	if (nFileTotalSize <= 0)
	{
		iResult = 115;
	...
	char strCtrlPort[32];
	char strFilePort[32];
	sprintf_s(strCtrlPort, "%d", m_nCtrlPort);
	sprintf_s(strFilePort, "%d", m_nFilePort);
...
void CUdtCore::ReplyAccept(const UDTSOCKET sock, const wchar_t* pstrReply)
{
    qDebug() << "ReplyAccept";
	CGuard::enterCS(m_Lock);
	for (vector<PCLIENTCONEXT>::iterator it = VEC_CLIENT.begin(); it != VEC_CLIENT.end(); it++)
	{
        qDebug() << "ReplyAccept1111";
		if ((*it)->sockAccept == sock)
		{
			char Head[8];
			memset(Head, 0, 8);
			if (wmemcmp(L"REJECTBUSY", pstrReply, 10) == 0 || wcslen(pstrReply) <= 0)
			{
				it = VEC_CLIENT.erase(it);
				memcpy(Head, "FRB", 3);
			}
			else if (wmemcmp(L"REJECT", pstrReply, 6) == 0)
			{
				it = VEC_CLIENT.erase(it);
				memcpy(Head, "FRR", 3);
			}
...
void CUdtCore::ProcessAccept(LISTENSOCKET * cxt)
{
	...
	sprintf_s(clienthost, "%s", inet_ntoa(clientaddr.sin_addr));
	sprintf_s(clientservice, "%d", clientaddr.sin_port);
...
int CUdtCore::ProcessSendCtrl(CLIENTCONEXT * cxt)
{
	...
	wstring szTmp, szFolderName, szFileName, szFilePath, szFinish = L"NETFAIL", szError = L"";
	string szTmp1;
	...
	if (nPos < 0)
	{
		nPos = szFilePath.find_last_of(L"\\");
	}
	szFileName = szFilePath.substr(nPos+1);
	szTmp = szFileName + L"\\" + WIN32_C2WC(cxt->ownDev.c_str()) + L"\\" + WIN32_C2WC(cxt->ownType.c_str()) + L"\\" + WIN32_C2WC(cxt->recvDev.c_str()) + L"\\" + WIN32_C2WC(cxt->recvType.c_str()) + L"\\" + WIN32_C2WC(cxt->sendType.c_str());
	if (cxt->vecFiles.size() < cxt->fileInfo.size())
		szTmp = szTmp + L"\\D\\";
	else
		szTmp = szTmp + L"\\F\\";
#ifdef WIN32
	szTmp1 = WIN32_WC2CU(szTmp.c_str());
#endif
	nLen = szTmp1.size();
	if (UDT::ERROR == UDT::send(client, (char*)&nLen, sizeof(int), 0))
		return 108;
	if (UDT::ERROR == UDT::send(client, szTmp1.c_str(), nLen, 0))
		return 108;
	...
	while (true)
	{
		...
		else if (memcmp(Head, "FPR", 3) == 0)
		{
			if (UDT::ERROR == UDT::recv(client, (char*)&nSendSize, sizeof(nSendSize), 0))
				return 108;
            qDebug() << "recv FPR" << nSendSize << " total size:" << nFileTotalSize;
			if (nSendSize <= nFileTotalSize && nSendSize > nLastSendSize)
			{
				nLastSendSize = nSendSize;
				CGuard::enterCS(m_Lock);
				double iProgress = cxt->iProgress;
				int64_t nTmp = 0;
				...
				if (cxt->bTransfer)
				{
					m_pCallBack->onTransfer(nFileTotalSize, nLastSendSize, iProgress, WIN32_WC2CU(szTmp.c_str()), 2, client);
				}
...
{
    qDebug() << "ProcessSendFile";
	char Head[8];
	int nLen = 0, nPos = 0, nFileCount = 0;
	int64_t nFileTotalSize = 0, nFileSize = 0, nSendSize = 0;
	wstring szTmp, szFolder, szAccept, szFileName, szFilePath, szFinish = L"NETFAIL", szError = L"";
	string szTmp1;
	...
	// send file
	for (vector<wstring>::iterator it2 = vecFile.begin(); it2 != vecFile.end(); it2++)
	{
		for (vector<_FileInfo>::iterator it3 = info.begin(); it3 != info.end(); it3++)
		{
			...
			if (szFilePath.compare(0, (*it2).size(), (*it2)) == 0)
			{
...
#ifdef WIN32
					szTmp1 = WIN32_WC2CU(szFolder.c_str());
#else
					szTmp1 = szFolder;
#endif
					nLen = szTmp1.size();
				...
				if (UDT::ERROR == UDT::send(client, (char*)Head, 3, 0))
					return 108;
#ifdef WIN32
				szTmp1 = WIN32_WC2CU(szFileName.c_str());
#else
				szTmp = szFileName;
#endif
				nLen = szTmp1.size();
				...
				if (nPos < 0)
				{
					nPos = szFileName.find_first_of(L"\\");
				}
				if (nPos >= 0)
				{
				...
				try
				{
					...
					if (ifs)
					{
						qDebug() << "open file success;" << szFilePath.c_str();
					}
					else
					{
						qDebug() << "open file error:" << szFilePath.c_str();
					}
					...
					while (left > 0)
					{
						...
						{
							if (VEC_CLIENT[i]->nCtrlFileGroup == cxt->nCtrlFileGroup)
							{
								VEC_CLIENT[i]->iProgress = trace.mbpsSendRate / 8;
								VEC_CLIENT[i]->fileName = szFileName;
							}
						}
						CGuard::leaveCS(m_Lock);
					}
					ifs.close();
					m_pCallBack->onAcceptonFinish(cxt->strClientAddr.c_str(), WIN32_WC2CU(szFileName.c_str()), 2, client);
				}
				catch (...)
				{
					return 108;
...
int CUdtCore::ProcessRecvFile(CLIENTCONEXT * cxt)
{
	...
	string szTmp, szTmp2, strReplyPath = "", szFinish = "NETFAIL", szSlash = "", szReFileName = "", szFolder = "";
	...
	while (true)
	{
		...
		else if (memcmp(Head,"MSR", 3) == 0 || memcmp(Head,"FSR", 3) == 0 || memcmp(Head,"DSR", 3) == 0)
		{
			...
			cxt->fileName = WIN32_CU2WC(szFileName.c_str());
		...
		else if (memcmp(Head, "MCS", 3) == 0)
		{
			...
			if (UDT::ERROR == UDT::recv(sock, pstrFileName, nLen, 0))
				return 108;
			pstrFileName[nLen] = '\0';
			wstring szRcvFileName;
#ifdef WIN32
			szRcvFileName = WIN32_CU2WC(pstrFileName);
#else
			...
			if (nCount <= 1)
			{
				if (szRcvFileName.find(L"/") != string::npos ||
					szRcvFileName.find(L"\\") != string::npos)
				{
					if (cxt->fileSavePath.find_last_of(L"/") == (cxt->fileSavePath.length() -1))
					{
						szFilePath = cxt->fileSavePath + szRcvFileName;
					}
					else
					{
						szFilePath = cxt->fileSavePath + L"/" + szRcvFileName;
					}			
				}
				else
				{
			...
			else
			{
				...
				{
#ifdef WIN32
					nRet = _waccess(szFilePath.c_str(), 0);
#else
					...
					if (nRet == 0)
					{
						...
						if (nPos > 0)
						{
							szFileName1 = szFilePath.substr(0, nPos);
							szTmp1 = szFilePath.substr(nPos);
						}
						else
						{
							szFileName1 = szFilePath;
							szTmp1= L"";
						}
						nLen = szFileName1.size();
						if (szFileName1[nLen-1] == ')')
						{
							if (szFileName1[nLen-3] == '(' && szFileName1[nLen-2] > '0' && szFileName1[nLen-2] <= '9')
							{
								if (szFileName1[nLen-2] < '9')
								{
									szFileName1[nLen-2] = szFileName1[nLen-2] + 1;
									szFilePath = szFileName1 + szTmp1;
								}
								else 
								{
									szFilePath = szFileName1.substr(0, nPos-2) + L"10)" + szTmp1;
								}
							}
							else if (szFileName1[nLen-4] == '(' && szFileName1[nLen-2] >= '0')
							{
								if (szFileName1[nLen-2] < '9')
								{
									szFileName1[nLen-2] = szFileName1[nLen-2] + 1;
							...
							szFilePath = szFileName1 + L"(1)" + szTmp1;
					}
				}while (nRet == 0);
			}
			...
			do 
			{
				...
				szTmp = '\\';
#endif
				nPos = szFilePath.find(WIN32_CU2WC(szTmp.c_str()), 0);
				if (nPos >= 0)
				{
					szFilePath.replace(nPos, 1, L"\\");
				}
			}while (nPos >= 0);
			...
			while(left > 0)
			{
				...
				//if (ctrl->bTransfer)
				{
					if (nResponseSize == 0 || nRecvSize - nResponseSize > TO_SND*1 || nFileTotalSize == nRecvSize)
					{
						memset(Head, 0, 8);
						memcpy(Head, "FPR", 3);
					...
					m_pCallBack->onTransfer(nFileTotalSize, nRecvSize, trace.mbpsRecvRate / 8, WIN32_WC2CU(szRcvFileName.c_str()), 1, sockCtrl);
				}
				CGuard::leaveCS(m_Lock);
			}
			ofs.close();
			...
			if (nPos < 0)
			{
				nPos = szRcvFileName.find_first_of(L"\\");
			}
			if (nPos >= 0)
			{
				...
				m_pCallBack->onAcceptonFinish(cxt->strClientAddr.c_str(), WIN32_WC2CU(szFilePath.c_str()), 1, sockCtrl);
		}
		else if (memcmp(Head,"DCR",3) == 0)
		{
			...
			if (cxt->fileSavePath.find_last_of(L"/") == (cxt->fileSavePath.length() -1))
			{
				szTmp1 = cxt->fileSavePath + szRcvFileName;
			}
			else
			{
				szTmp1 = cxt->fileSavePath + L"/" + szRcvFileName;
			}
			//CreateDirectroy(szTmp);
			UdtFile::CreateDir(szTmp1.c_str());
		}
		else if (memcmp(Head,"FSC",3) == 0)
		{
			memset(Head, 0, 8);
