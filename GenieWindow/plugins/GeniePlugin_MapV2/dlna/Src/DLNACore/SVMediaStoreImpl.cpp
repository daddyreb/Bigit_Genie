#include "SVMediaStoreImpl.h"

/*----------------------------------------------------------------------
|   logging
+---------------------------------------------------------------------*/
NPT_SET_LOCAL_LOGGER("SiteView.MediaStore")
...
static const MSMimeInfo g_mimeInfo[] = {
	...
	{ ".ico", "image/ico",      MSMediaType_Image, ".ico" },
	{ ".pcx", "image/pcx",      MSMediaType_Image, ".pcx" },
...
void MediaStoreImpl::importFileSystem(const NPT_List<NPT_String>& dirs, const NPT_List<NPT_String>& names, bool ignoreDot, ShareType eType)
{
	m_taskGroup->startTask(new MSImportTask(this, dirs, names, ignoreDot, eType));
}
...
void MediaStoreImpl::getImportResult(NPT_List<MSImportResult>& ls)
{
	ls = m_MSResultList;
}
...
MSImportTask::MSImportTask(MediaStoreImpl *owner, const NPT_List<NPT_String>& dirs, const NPT_List<NPT_String>& names, bool ignoreDot, ShareType eType)
	: m_owner(owner), m_dirs(dirs), m_names(names), m_ignoreDot(ignoreDot), m_eType(eType)
{
}
...
void MSImportTask::exec()
{
	...
	for (NPT_Ordinal i = 0; i < m_dirs.GetItemCount(); i++) {
		if (!aborted()) {
			...
			{
				import(impCtx, impCtx->m_folder, impCtx->rootPath, fileInfo, m_eType);
...
void MSImportTask::import(ImportContext *impCtx, MSFolder *parentFolder, const NPT_String& dir, const NPT_FileInfo& dirInfo, ShareType eType)
{
	try
	{
				...
				if (!aborted()) {
					...
					if (NPT_SUCCEEDED(NPT_File::GetInfo(fullPath, &fileInfo))) {
						if (fileInfo.m_Type == NPT_FileInfo::FILE_TYPE_DIRECTORY && (!m_ignoreDot || !fname.StartsWith("."))) {
							...
							import(impCtx, subFolder, fullPath, fileInfo, eType);
							if (subFolder->childCount() > 0) {
							...
							importFile(impCtx, parentFolder, fullPath, fileInfo, dir, eType);
						}
					}
				} else {
					break;
...
void MSImportTask::importFile(ImportContext *impCtx, MSFolder *parentFolder, const NPT_String& path, const NPT_FileInfo& fileInfo, const NPT_String& dir, ShareType eType)
{
	NPT_String ext = NPT_FilePath::FileExtension(path);
	ext.MakeLowercase();
	const MSMimeInfo **mimeInfo;
	if (NPT_SUCCEEDED(g_mimeIndexByExt.Get(ext, mimeInfo)))
	{
		...
		{
		case MSMediaType_Video:
			if (eType == ShareType_Video || eType == ShareType_VA || eType == ShareType_VAI)
			{
				item->m_upnpClass = "object.item.videoItem";
				item->m_mimeType = (*mimeInfo)->mimeType;
			}
			...
			break;
		case MSMediaType_Audio:
			if (eType == ShareType_Audio || eType == ShareType_VA || eType == ShareType_VAI)
			{
				item->m_upnpClass = "object.item.audioItem";
				item->m_mimeType = (*mimeInfo)->mimeType;
				item->m_artistList.Add("Unknown Artist");
			...
			break;
		case MSMediaType_Image:
			if (eType == ShareType_Image || eType == ShareType_VI || eType == ShareType_VAI)
			{
				item->m_upnpClass = "object.item.imageItem.photo";
				item->m_mimeType = (*mimeInfo)->mimeType;
			}
