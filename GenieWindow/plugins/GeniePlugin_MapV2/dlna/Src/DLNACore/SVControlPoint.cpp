#include "SVControlPoint.h"
#include "SVDescPriv.h"

/*----------------------------------------------------------------------
|   logging
+---------------------------------------------------------------------*/
NPT_SET_LOCAL_LOGGER("SiteView.DeviceImpl")
...
bool ControlPoint::SearchContext::ssdpSearchTaskOnPacket(SsdpSearchTask *task, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr)
{
	m_owner->processSsdpSearchResponse(m_ifctx, packet, fromAddr);
	return false;
}
...
ControlPoint::DiscoveryContext::DiscoveryContext(ControlPoint *owner, const FrontEnd::InterfaceContext *ifctx, const NPT_String& server, const NPT_String& xmpp, int maxAge)
	: m_owner(owner), m_ifctx(ifctx), m_server(server), m_xmpp(xmpp), m_maxAge(maxAge)
{
}
...
{
	m_owner->processDiscoveryResult(nr, m_ifctx, task->location(), m_server, m_xmpp, m_maxAge, rootDeviceDesc, uuid);
}
...
void ControlPoint::ssdpDiscoverRootDevice(const FrontEnd::InterfaceContext *ifctx, const UUID& uuid, const NPT_String& location, const NPT_String& server, const NPT_String& custom, const NPT_String& fromAddr, int maxAge)
{
	WriteLocker locker(m_stateLock);
	if (m_state != State_Attached) {
		return;
	...
	//NPT_String local = location.SubString(7);	// http://172.16.0.12:5414/devices/
	//int nPos = local.Find(':');
	//local = local.Left(nPos);

	DiscoverDevInfo **dev;
	if (NPT_SUCCEEDED(m_DisDevMap.Get(uuid, dev)))
	{
		DiscoverDevInfo * disinfo = * dev;
		if (NPT_String::CompareN(disinfo->m_fromAddr, fromAddr, disinfo->m_fromAddr.GetLength()) == 0)
		{
			NPT_System::GetCurrentTimeStamp(disinfo->m_updateTS);
		}
		NPT_List<DiscoverDevInfo*>::Iterator it = m_DisDevList.Find(DisDevInfoFinder(fromAddr));
		if (!it)
		{
			DiscoverDevInfo * devinfo = new DiscoverDevInfo();
			m_DisDevList.Add(devinfo);
			//m_DisDevMap.Put(uuid, devinfo);
			devinfo->m_uuid = uuid;
			devinfo->m_location = location;
			devinfo->m_server = server;
			devinfo->m_fromAddr = fromAddr;
			devinfo->m_custom = custom;
			NPT_System::GetCurrentTimeStamp(devinfo->m_registerTS);
			devinfo->m_updateTS = devinfo->m_registerTS;
			if (m_callback)
			{
				bool subscribe = true;
				m_callback->controlPointOnDiscovery(this, devinfo, subscribe);
			}
		}
		else
		{
			NPT_System::GetCurrentTimeStamp((*it)->m_updateTS);
		}
	}
	else
	{
	...
	DeviceDiscoveryTask *discoveryTask = new DeviceDiscoveryTask(location, uuid, new DiscoveryContext(this, ifctx, server, custom, maxAge), true);
	m_pendingSsdpDiscoverTasks.Put(uuid, discoveryTask);
	m_attachedTaskGroup->startTask(discoveryTask);
}

void ControlPoint::processDiscoveryResult(NPT_Result nr, const FrontEnd::InterfaceContext *ifctx, const NPT_String& location, const NPT_String& server, const NPT_String& custom, int maxAge, DeviceDesc *rootDeviceDesc, const UUID& uuid)
{
...
void ControlPoint::ssdpRemoveRootDevice(const UUID& uuid, const NPT_String& custom)
{
	WriteLocker locker(m_stateLock);
	if (m_state != State_Attached) {
		return;
	...
	if (custom.IsEmpty())
	{
		...
		if (NPT_SUCCEEDED(m_DisDevMap.Get(uuid, dev)))
		{
			DiscoverDevInfo *dis = *dev;
			m_DisDevList.Remove(dis);
			m_DisDevMap.Erase(uuid);

			for (NPT_Ordinal j = 0; j < m_DisDevList.GetItemCount(); j++)
			{
				NPT_List<DiscoverDevInfo*>::Iterator iter = m_DisDevList.GetItem(j);
				DiscoverDevInfo * dis1 = *iter;
				if (dis1->m_uuid == dis->m_uuid)
				{
					m_DisDevList.Remove(dis1);
					if (m_callback)
					{
						bool subscribe = false;
						m_callback->controlPointOnDiscovery(this, dis1, subscribe);
					}
					break;
				}
			}
			...
			{
				bool subscribe = false;
				m_callback->controlPointOnDiscovery(this, dis, subscribe);
			}
		}
	}
}
...
void ControlPoint::removeExpiredDevicesLoop()
{
	for (;;)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// remove expired discovery devices
		{
			WriteLocker locker(m_stateLock);
			for (NPT_Ordinal i = 0; i < m_DisDevList.GetItemCount(); i++)
			{
				NPT_List<DiscoverDevInfo*>::Iterator it = m_DisDevList.GetItem(i);
				DiscoverDevInfo * dev = *it;
				if (tsMillis - dev->m_updateTS.ToMillis() > 60000)
				{
					deleteDiscoveryDevice(dev);
					m_DisDevList.Erase(it);
					bool bAsolHas = false;
					for (NPT_Ordinal j = 0; j < m_DisDevList.GetItemCount(); j++)
					{
						NPT_List<DiscoverDevInfo*>::Iterator iter = m_DisDevList.GetItem(j);
						if ((*iter)->m_uuid == dev->m_uuid)
						{
							bAsolHas = true;
							break;
						}
		...
		// remove expired root devices
		{
			...
			for (NPT_Ordinal i = 0; i < count; )
			{
				...
				if (tsMillis - rootDevice->m_updateTS.ToMillis() > 120000)
				{
					m_rootDeviceList.Erase(it);
					m_rootDeviceIndexByUUID.Erase(rootDevice->m_deviceDesc->uuid());
					deleteRootDevice(rootDevice);
...
void ControlPoint::deleteDiscoveryDevice(DiscoverDevInfo* dis)
{
	if (m_callback)
	{
		bool subscribe = false;
...
void ControlPoint::onAddDevice(DeviceDesc *deviceDesc, RootDevice *rootDevice)
{
	...
	DeviceDescPrivate::getPrivate(deviceDesc)->m_xmpp = rootDevice->m_xmpp;
...
ControlPoint::ProcessSsdkNotifyTask::ProcessSsdkNotifyTask(ControlPoint *owner, const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr)
	: m_owner(owner), m_ifctx(ifctx), m_packet(packet), m_fromAddr(fromAddr)
{
}

void ControlPoint::ProcessSsdkNotifyTask::exec()
{
	m_owner->processSsdpNotifyReal(m_ifctx, m_packet, m_fromAddr);
}

void ControlPoint::processSsdpNotify(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr)
{
	this->m_attachedTaskGroup->startTask(new ProcessSsdkNotifyTask(this, ifctx, packet, fromAddr));
}

void ControlPoint::processSsdpNotifyReal(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr)
{
	NPT_HttpRequest *req;
	...
	if (NPT_SUCCEEDED(NPT_HttpRequest::Parse(xmlInputStream, NULL, req))) {
		...
		do {
			...
			NPT_String xmpp = "";
			NPT_HttpHeader *hdrXMPP = req->GetHeaders().GetHeader("CUSTOM");
			if (hdrXMPP)
			{
				xmpp = hdrXMPP->GetValue();
			}
			...
			if (hdrNTS->GetValue().Compare("ssdp:alive") == 0) {
				...
				ssdpDiscoverRootDevice(ifctx, rootDeviceUuid, hdrLocation->GetValue(), hdrServer->GetValue(), xmpp, fromAddr.GetIpAddress().ToString(), maxAge);

			} else if (hdrNTS->GetValue().Compare("ssdp:byebye") == 0) {
				ssdpRemoveRootDevice(rootDeviceUuid, xmpp);
			} else {
				break;
			}
...
void ControlPoint::processSsdpSearchResponse(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr)
{
	NPT_HttpResponse *resp;
	NPT_InputStreamReference inputStream(new NPT_MemoryStream(packet.GetData(), packet.GetDataSize()));
	NPT_BufferedInputStream xmlInputStream(inputStream);
	if (NPT_SUCCEEDED(NPT_HttpResponse::Parse(xmlInputStream, resp)))
	{
		...
		do {
			...
			NPT_String xmpp = "";
			NPT_HttpHeader *hdrXMPP = resp->GetHeaders().GetHeader("CUSTOM");
			if (hdrXMPP)
			{
				xmpp = hdrXMPP->GetValue();
			}
...
void ControlPoint::processHttpRequest(AbortableTask *task, const FrontEnd::InterfaceContext *ifctx, const NPT_String& relPath, const FrontEnd::RequestContext& reqCtx, const NPT_HttpRequest *req, NPT_HttpResponse& resp, NPT_InputStream *inputStream, HttpOutput *httpOutput)
{
	do {
...

		{
			...
			ExtraServiceInfo *matchExtraServiceInfo = NULL;
