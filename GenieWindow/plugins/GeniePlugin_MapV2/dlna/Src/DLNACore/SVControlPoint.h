#ifndef __SV_CONTROLPOINT_H__
#define __SV_CONTROLPOINT_H__

#include "SVFrontEnd.h"
#include "SVTasks.h"
#include "SVDesc.h"
...
public:
	...
	{
	public:
		virtual void controlPointOnDiscovery(ControlPoint *controlPoint, DiscoverDevInfo * info, bool & subscribe) = 0;
...
private:
	...
	void processSsdpNotify(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr);
	void processSsdpNotifyReal(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr);
	void processSsdpSearchResponse(const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr);
	...
	void ssdpDiscoverRootDevice(const FrontEnd::InterfaceContext *ifctx, const UUID& uuid, const NPT_String& location, const NPT_String& server, const NPT_String& custom, const NPT_String& fromAddr, int maxAge);
	void processDiscoveryResult(NPT_Result nr, const FrontEnd::InterfaceContext *ifctx, const NPT_String& location, const NPT_String& server, const NPT_String& custom, int maxAge, DeviceDesc *rootDeviceDesc, const UUID& uuid);
	void ssdpRemoveRootDevice(const UUID& uuid, const NPT_String& custom);
	...
	void deleteDiscoveryDevice(DiscoverDevInfo* dis);
	...
	{
	public:
		ProcessSsdkNotifyTask(ControlPoint *owner, const FrontEnd::InterfaceContext *ifctx, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr);
		virtual void exec();
	...
	protected:
		virtual bool ssdpSearchTaskOnPacket(SsdpSearchTask *task, const NPT_DataBuffer& packet, const NPT_SocketAddress& fromAddr);
	...
	{
	public:
		DiscoveryContext(ControlPoint *owner, const FrontEnd::InterfaceContext *ifctx, const NPT_String& server, const NPT_String& xmpp, int maxAge);
...
private:
	...
	NPT_List<DiscoverDevInfo*> m_DisDevList;
	NPT_Map<UUID, DiscoverDevInfo*> m_DisDevMap;
