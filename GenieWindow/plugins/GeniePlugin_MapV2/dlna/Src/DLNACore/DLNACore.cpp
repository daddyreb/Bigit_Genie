...
void DLNACore::importFileSystemToMediaServer(const NPT_List<NPT_String>& dirs, const NPT_List<NPT_String>& names, bool ignoreDot, ShareType eType)
{
	mediaStore()->importFileSystem(dirs, names, ignoreDot, eType);
}
...
NPT_Result DLNACore::snapshotDiscoveryList(NPT_List<DiscoverDevInfo*> & disList)
{
	m_impl->snapshotDiscoveryList(disList);
	return NPT_SUCCESS;
}
