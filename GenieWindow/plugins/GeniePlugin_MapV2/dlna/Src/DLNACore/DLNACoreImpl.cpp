...
|   logging
+---------------------------------------------------------------------*/
NPT_SET_LOCAL_LOGGER("SiteView.DLNACore")
...
void DLNACoreImpl::setProperty(const NPT_String& name, const NPT_String& value)
{
	...
	} else if (name.Compare("CUSTOM", true) == 0) {
		m_fe->setDevName(value);
...
NPT_Result DLNACoreImpl::snapshotDiscoveryList(NPT_List<DiscoverDevInfo*> & disList)
{
	ReadLocker locker(m_stateLock);
	disList = m_DevList;
	return NPT_SUCCESS;
...
void DLNACoreImpl::controlPointOnDiscovery(ControlPoint *controlPoint, DiscoverDevInfo * info, bool & subscribe)
{
	WriteLocker locker(m_stateLock);
	// add device
	if (subscribe)
	{
		NPT_List<DiscoverDevInfo*>::Iterator it = m_DevList.Find(DisDevInfoFinder(info->m_fromAddr));
		if (!it)
		{
			m_DevList.Add(info);
			invokeDelegate(DelegateMethodIndex_onDiscoveryListChanged, info->m_fromAddr, info->m_custom, 0);
		}
	}
	else
	{
		...
		{
			m_DevList.Erase(it);
			invokeDelegate(DelegateMethodIndex_onDiscoveryListChanged, info->m_fromAddr, info->m_custom, 1);
		}
	}
}
...
void DLNACoreImpl::invokeDelegate(int index, const NPT_String& ip, const NPT_String& devName, const int type)
{
	if (m_delegate) {
		m_taskGroup->startTask(new InvokeDelegateMethodTask(m_delegate, index, ip, devName, type));
	}
}
...
InvokeDelegateMethodTask::InvokeDelegateMethodTask(DLNACoreDelegate *d, int methodIndex, const NPT_String& ip, const NPT_String& devName, const int type)
	: m_delegate(d), m_methodIndex(methodIndex), m_deviceDesc(NULL), m_ip(ip), m_devName(devName), m_Type(type)
{
}
...
void InvokeDelegateMethodTask::exec()
{
	...
	case DLNACoreImpl::DelegateMethodIndex_onDiscoveryListChanged:
		m_delegate->onDiscoveryListChaned(m_ip, m_devName, m_Type);
		break;
