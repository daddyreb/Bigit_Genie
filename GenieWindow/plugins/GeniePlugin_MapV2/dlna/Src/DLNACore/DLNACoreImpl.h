...
public:
	...
	enum DelegateMethodIndex
	{
		...
		DelegateMethodIndex_onDiscoveryListChanged,
	};
	...
	NPT_Result snapshotDiscoveryList(NPT_List<DiscoverDevInfo*> & disList);
...
protected:
	virtual void controlPointOnDiscovery(ControlPoint *controlPoint, DiscoverDevInfo * info, bool & subscribe);
...
private:
	...
	void invokeDelegate(int index, const NPT_String& ip, const NPT_String& devName, const int type);
	...
	NPT_List<DiscoverDevInfo*> m_DevList;
...
public:
	...
	InvokeDelegateMethodTask(DLNACoreDelegate *d, int methodIndex, const NPT_String& ip, const NPT_String& devName, const int type);
